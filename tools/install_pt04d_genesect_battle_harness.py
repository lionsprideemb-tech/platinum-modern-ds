...
def main() -> None:
    ...
    parser.add_argument("--report", type=Path, default=Path("pt04d-genesect-battle-harness.json"))
    ...
    if "Encounter_NewVsSpeciesAtLevel" not in text:
        raise SystemExit(
            "PT04D Genesect battle harness expects the completed PT04C battle-entry harness first"
        )
    ...
    report = {
        ...
        "expected_visual_proof": [
            "Genesect player-side back sprite",
            "GENESECT player name",
            "Lv.50 and sane HP",
            "native battle command UI",
            "stable render across multiple captures",
        ],
        "next_if_passed": "seal PT04D Gen V runtime expansion and move to next canonical generation batch",
    }
