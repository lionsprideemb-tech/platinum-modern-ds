...
def patch_title_runtime(source: Path):
    ...
    # The full 256x192 Mercury lower scene uses 24 KiB of BG3 character
    # data at Platinum's original 0x0000 base. Keep that proven mapping intact
    # and move the tiny copyright BG1 tiles out of its way instead.
    copyright_bg_old = (
        "    BgTemplate bgMain1 = {\n"
        "        .x = 0,\n"
        "        .y = 0,\n"
    ...
    if copyright_bg_old not in text:
        raise RuntimeError("could not locate Platinum copyright background template")
    text = text.replace(copyright_bg_old, copyright_bg_new, 1)
