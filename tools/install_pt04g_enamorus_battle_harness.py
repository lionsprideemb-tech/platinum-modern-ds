...
def main() -> None:
    ...
    parser.add_argument("--report", type=Path, default=Path("pt04g-enamorus-battle-harness.json"))
    ...
    if "Encounter_NewVsSpeciesAtLevel" not in text:
        raise SystemExit(
            "PT04G Enamorus battle harness expects the completed PT04C battle-entry harness first"
        )
    ...
    report = {
        ...
        "expected_visual_proof": [
            "Enamorus player-side back sprite",
            "ENAMORUS player name",
            "Lv.50 and sane HP",
            "native battle command UI",
            "stable render across multiple captures",
        ],
        "next_if_passed": "seal PT04G Gen VIII runtime expansion; only then begin Generation IX",
    }
